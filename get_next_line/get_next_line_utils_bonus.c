...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/28 09:35:16 by andorako          #+#    #+#             */
/*   Updated: 2024/03/28 09:35:17 by andorako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_push_backb(t_list *l_begin, char *str, int end)
{
	...
	new_node = (t_list_node *)ft_allocb(sizeof(t_list_node));
	if (!new_node)
	{
		ft_cleanb(l_begin, 1);
		return ;
...
char	*ft_joinb(t_list *l_begin)
{
	...
	total_len = ft_strs_sizeb(l_begin->head);
	res = (char *)ft_allocb((total_len + 1) * sizeof(char));
	if (!res)
		return (ft_cleanb(l_begin, 1));
	res[total_len] = '\0';
	curr = l_begin->head;
	check = 0;
	total_len = 0;
...
char	*ft_subb(char *str, int length)
{
	...
	ans = (char *)ft_allocb((length + 1) * sizeof(char));
	if (ans)
	{
		idx = -1;
		ans [length] = '\0';
...
int	ft_strs_sizeb(t_list_node *curr)
{
	int		total_len;
	int		check;
	int		len;
